...
| `/ge-status [r]` | validate, render, open `status.html` (live from then on); ready / in progress / last 3 ledger rows / open calls |
...
`ge/<r>/next.md` — the kickoff for the next node; first line `# kickoff: <id>`. `ledger.md` — `| date | session | task | event | outcome | commit |`, append-only (`done`, `re-closed`, `miss`, `blocked`, `paused <reason>`, `resumed`, `revised`, `review`, and `started` per dispatch, which `ledger` hides unless `--all`); rows carry date and time; a node keeps exactly one `done` row, and a close repeated with a different hash appends `re-closed` naming the hash it replaced. `calls.md` — `## Open` / `## Decided`. `status.html` — the dashboard: self-contained, drawn from one JSON of the roadmap, live through a sibling `status.js` it polls every 2 s (no server, no reload), every node expandable to its kickoff, gate activity, completion summary or block reason, the graph pannable and zoomable, dark and light, with a `STOPPED` or `PAUSED: <reason>` banner while a hold is on; every verb that changes state redraws it. `status.js` and `activity.log` (gate runs as they happen) are gitignored by a `ge/.gitignore` the first render writes. `summaries/` — pause summaries. `PAUSE` / `STOP` — present while paused / stopped.
