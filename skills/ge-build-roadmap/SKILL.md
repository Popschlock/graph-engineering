...
`GE init <r> --goal "<goal>"`; put the phases under `## Phases` in `ge/<r>/roadmap.md` (`### <id> — <title>` + paragraph); `GE add <r> --id <id> --subject "<subject>" --deps "<ids>" --spec "<spec>" --gate "<names>"` per task in order (`--deps` and `--gate` are comma-separated lists); `GE validate <r>` until `OK`. Write `ge/<r>/next.md` for `GE next <r>`'s node — `GE next <r>` prints `<id> | <subject> | <gates>`, so the node's id is the first pipe-separated field: first line `# kickoff: <id>`, then a complete kickoff for a session that knows nothing (what to read, what to build, the pins or tests, its gate names, how to close). If `GE next <r>` prints `none`, the first line is `# kickoff: none`. `GE validate <r>` again, until `OK`. `GE render <r>`. `git add ge && git commit -m "ge(<r>): roadmap — <the goal's first words>"`. Say the next node and `/ge-run-roadmap <r> dry` to see its brief.
