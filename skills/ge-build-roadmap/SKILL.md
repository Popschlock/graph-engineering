...
**Size a task to one build-and-verify cycle, not to one idea.** Every row costs a fixed overhead whatever it contains: a fresh agent reading the roadmap and its kickoff, the gates run twice, an editor closed and reopened where a build needs that, a verifier, a kickoff for what it unlocks. A run measured on one project spent 56% of a week's tokens in task agents and most of that on rows that each proved one small change with a long live-measurement transcript. So a row is the largest set of RELATED changes that one build, one editor session and one verification can cover, and low-risk changes that share a lock set travel together. The docs, handoff and change-log lines for a change belong to the row that makes it, written by the agent that has the numbers in context; a separate docs row exists only when the docs ARE the deliverable. A `READ:` row exists only when its answer changes the graph (which rows exist, their order, their locks); a fact one row needs is a line in that row's kickoff, not a row. Split a row only for a different lock set, a genuinely risky change that must be verified alone, or a human call in the middle.

**Name the proof level in every kickoff.** `smoke`: gates green plus one live run of the user-facing path, then close — the human's playtest is the gate, so a roadmap that pauses for `human-test` after each phase runs `smoke` rows. `measure`: the numbers are the deliverable (a perf budget, a replication cost), so the kickoff lists exactly which numbers and the agent measures those and nothing else. A kickoff that names no level is `smoke`.
